...
static bool renderSystemInitialized = false;
...
void updateNodeDescriptor(Renderer* a_pRenderer, Node* node, uint32_t& nodeCounter)
{
	if (node->mesh)
	{
	...
	for (Node* child : node->children)
		updateNodeDescriptor(a_pRenderer, child, nodeCounter);
}
...
void RenderSystem::Init(IApp* a_pApp)
{
	pCamera = new Camera();
...
void RenderSystem::Load()
{
	if (pRenderer->window.reset)
	{
		...
		for (Node* node : scene.nodes)
			updateNodeDescriptor(pRenderer, node, nodeCounter);
